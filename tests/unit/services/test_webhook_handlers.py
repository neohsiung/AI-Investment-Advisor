...
# TestHandleStripeWebhook lived here. The Stripe webhook is gone: it created
# user accounts from checkout emails, which a single-owner deployment must
# never do. The surviving webhook auth path (X-API-Key -> owner) is covered
# by TestHandleGenericWebhook above and tests/unit/core/test_webhook_auth_negative.py.
# Stripe webhook 已移除（會依結帳 email 建立帳號）；剩餘的 webhook 驗證路徑
# 由上方 TestHandleGenericWebhook 與 test_webhook_auth_negative.py 覆蓋。
