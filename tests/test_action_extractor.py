...
def test_action_extractor_json_block():
    """
...
The target price is $200.'''
    ...
    with unittest.mock.patch.object(agent, 'run_tool_loop', return_value=mock_content):
        result = agent.run("Dummy council text")
...
def test_action_extractor_unstructured_chinese():
    """
    ...
    agent = ActionExtractorAgent(tier="fast")
    with unittest.mock.patch.object(agent, 'run_tool_loop', return_value=mock_content):
        result = agent.run("輝達現在太貴了，建議賣出5股，信心分數8分。")
...
def test_action_extractor_invalid_output():
    """
    ...
    agent = ActionExtractorAgent(tier="fast")
    with unittest.mock.patch.object(agent, 'run_tool_loop', return_value=mock_content):
        result = agent.run("Garbage input")
