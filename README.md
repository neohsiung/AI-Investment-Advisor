...
- **⚖️ 專業級風控與資安硬化 (Institutional Risk & Security Hardening)**: 獨有的槓桿引擎與全域脫敏機制 (`BaseAgent.redact_secrets`)。系統現已全面採用 SHA256 進行敏感 ID 生成，並強制執行依賴項漏洞審計。
...
- **⚖️ Institutional Risk & Security Hardening (Precision Leverage & Redaction)**: Professional-grade tracking of Gross/Net NLV and margin utilization. Features centralized secret redaction and SHA256 cryptographic upgrades.
