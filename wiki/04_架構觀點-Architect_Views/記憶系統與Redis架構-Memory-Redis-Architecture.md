...
> [!NOTE]
> 採用三層式記憶過濾架構，確保反應速度與知識深度的完美平衡。

```mermaid
graph TD
    subgraph Hot [🚀 Hot Memory - Cache Tier]
        Redis[(Redis)]
        ScopeH[ResponseCache, Session State]
        TTLH[TTL: 24h - 7d]
    end
    
    subgraph Warm [🧠 Warm Memory - Persistent Tier]
        PG[(PostgreSQL + pgvector)]
        ScopeW[RAG, Reranking, Preferences]
        TTLW[Permanent]
    end
    
    subgraph Cold [❄️ Cold Memory - Source Tier]
        CSV[(Raw Files / CSV)]
        ScopeC[Ingestion Sources]
    end

    Hot -.->|Cache Miss / Retrieval| Warm
    Warm -.->|Initial Ingestion| Cold
    
    style Hot fill:#f9f,stroke:#333,stroke-width:2px
    style Warm fill:#bbf,stroke:#333,stroke-width:2px
```
...
```mermaid
sequenceDiagram
    participant GW as Gateway/Workflow
    participant MS as MemoryService
    participant A as Agent
    participant DB as DB / MEMORY.md
    
    GW->>MS: Check Context Limit
    alt Approaching 80% or <4000 tokens left
        MS->>GW: Trigger Reserve Floor
        GW->>A: Silent Pre-Compaction Flush Prompt
        A->>DB: Save critical state & keywords
        A-->>GW: Return NO_REPLY
        GW->>MS: Archive older 80% to DB
        MS->>MS: Retain recent 20% raw history
    end
```
