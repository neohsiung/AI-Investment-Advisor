...
`src/cli.py` is the main entry point for the application. It replaces legacy scripts like `scheduler.py` and `workflow.py`, providing a unified interface to run various system components.
...
Runs a background daemon that automatically executes daily checks, weekly reports, and validation tasks based on the schedule.
...
#### 2. Daily Check (`daily`)
Manually triggers a Daily Portfolio Scan for a specific user.
...
Manually triggers the Weekly Analysis Report generation process.

- **Dry Run**: Generates report and logs only, **NO** Email sent.
- **Normal**: Generates and sends Email report.
...
#### 4. Backtest (`backtest`)
Runs a backtest simulation for a specific Ticker, generating performance metrics and feedback data.
