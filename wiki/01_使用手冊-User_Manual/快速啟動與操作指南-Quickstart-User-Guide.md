...
| 2026-02-15 | v3.4 | Added LINE Bot and Sentinel monitoring setup. | Neo |
| 2026-01-10 | v3.1 | Initial Quickstart guide for v3.0 specs. | Neo |
...
```mermaid
graph LR
    User[使用者/Webhook] -->"|訊號/輸入| DM[系統入口]"
    DM -->"|分析| Swarm[Agent Swarm Clusters]"
    Swarm -->|交易提案| ATS{AutomatedTradingService}
    ATS -->"|Confidence >= Threshold| Exec[自動下單]"
    ATS -->"|Confidence < Threshold| Appr[等待人工審批]"
    
    subgraph "Evolution Loop"
        Swarm -->"|績效回饋| Eng[SystemEngineerAgent]"
        Eng -->|Alpha 基因演算| Swarm
    end
```
...
- **交易配置 (Trading Setup)**:
    ...
    - **Auto-Trade Threshold (自動交易信心閾值)**: 預設為 9。當 Agent 發出的交易指令信心分數大於等於此數值時，系統將**自動執行交易** (免審批)。分數較低者將推送至 LINE/Email 請求人工審批 (限時 5 分鐘)。
- **AI 配置**: 設定 Provider (Gemini, OpenRouter, OpenAI) 與 **Model Tiering**。
    - **Smart/Advanced Tier**: 用於複雜邏輯推論與 SystemEngineer 的策略回測。
    - **Fast Tier**: 用於快速訊息過濾與 Sentinel 哨兵掃描。
...
sequenceDiagram
    ...
    participant ATS as AutoTradingService
    
    TV->>WHS:"POST /webhook/tradingview (Payload: BUY/SELL)"
    WHS->>WHS: Verify Security Token
    
    WHS->>SEN:"_trigger_emergency_protocol(event)"
    
    alt is_extreme & action == 'SELL'
        SEN->>ATS: Evaluate Emergency Liquidation
        ATS-->>WHS: Trade Executed / Approval Requested
    else Normal Signal
        SEN->>Council: Wake up for Debate
        Council-->>WHS: Deep Analysis Report
    end
```
...
1. **設定安全金鑰**:
   ...
   FINNHUB_WEBHOOK_SECRET=your_finnhub_secret
   ```
2. **暴露本地端點 (若在本地執行)**:
   執行 `ngrok http 8000` 取得對外 HTTPS URL (如 `https://xxx.ngrok-free.app`)。如果您部署於雲端 (Cloud Run)，則直接使用該網域。
3. **在 TradingView 設定 Alert**:
   - 建立新快訊 (Alert)。
   - 在 `Webhook URL` 填入: `https://<YOUR_DOMAIN>/webhook/tradingview`
   - 在 `Message` 區域填入 JSON 格式，必須包含以下欄位與 Header `x-webhook-secret`:
   ```json
   {
     "symbol": "AAPL",
     "action": "SELL",
   ...
   ```
   *注意*: 發送請求時請確保攜帶 HTTP Header `x-webhook-secret: your_secure_random_string` 進行身分驗證。
