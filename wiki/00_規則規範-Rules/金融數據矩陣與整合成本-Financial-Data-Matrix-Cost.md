...
| :--- | :--- | :--- | :--- |
| 2026-02-27 | v1.8 | 實作 `PolygonStreamClient` (WebSocket) 與 OTel 自動監控 | Antigravity |
| 2026-02-27 | v1.7 | 實作 `TiingoProvider` 並優化 `PolygonProvider` 批次快照 | Antigravity |
| 2026-02-21 | v1.6 | 新增「實作狀態」欄位，標註 Provider 實際實作情況 | Antigravity |
...
> **實作狀態圖例**：✅ 已實作（`src/data/providers/` 中有對應 Provider）| 🔜 規劃中（有 Service 但無獨立 Provider）| ❌ 未實作（僅列於規劃）

### 1. 核心矩陣 (Core Matrix)

| 優先序 | 來源 | 類別 | 方法 | 優點 | 性價比 | 實作狀態 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **P0** | **Fred** | 總經 | REST | 權威指標 | 極高 | ✅ `fred_provider.py` (v5.0) |
| **P0** | **Finnhub** | 情緒 | REST | 情感分析強 | 高 | ✅ `finnhub_provider.py` (v5.0) |
| **P0** | **FMP** | 財報 | REST | 財報數據準 | 高 | ✅ `fmp_provider.py` |
| **P1** | **Futu** | 交易/行情 | Gateway | 即時、支援交易 | 高 | 🔜 有 `futu_service.py` 但無獨立 Provider |
| **P1** | **Polygon** | 行情 | REST/WS | 即時、支援 v3 快照與 WebSocket 串流 | 中 | ✅ `polygon_provider.py` |
| **P1** | **Twelve Data** | 綜合 | REST | 全球覆蓋 | 高 | ❌ 未實作 |
| **P1** | **Yahoo** | 行情 | Library | 免費、搜尋強 | 極高 | ✅ `yfinance_provider.py` |
| **P1** | **Alpaca** | 交易 | SDK | 零佣金 API | 中 | ❌ 未實作 |
| **P2** | **Alpha Vantage**| 指標 | REST | 總經+情緒 | 中 | ✅ `alpha_vantage_provider.py` (v5.0) |
| **P2** | **Tiingo** | 新聞 | REST | 內容乾淨、標籤強 | 高 | ✅ `tiingo_provider.py` |
| **P2** | **NewsAPI** | 新聞 | REST | 全球來源廣 | 低 | ❌ 未實作 |
| **P2** | **CryptoPanic** | 加密情緒 | REST | 幣圈聚合 | 高 | ❌ 未實作 |
| **P2** | **Alternative.me**| 市場恐懼 | REST | 指標化避險 | 高 | ❌ 未實作 |
| **P2** | **Whale Alert** | 大額異動 | Webhook | 鏈上預警 | 低 | ❌ 未實作 |
| **P2** | **Glassnode** | 鏈上總經 | REST | 週期分析 | 中 | ❌ 未實作 |

#### 實作狀態摘要

| 狀態 | 數量 | 來源 |
| :--- | :--- | :--- |
| ✅ 已實作 | 7 | FMP, Polygon, Yahoo Finance, Tiingo, FRED, Finnhub, Alpha Vantage |
| 🔜 規劃中 | 1 | Futu (有 Service) |
| ❌ 未實作 | 7 | Twelve Data, Alpaca, NewsAPI, CryptoPanic, Alternative.me, Whale Alert, Glassnode |

> **已實作的 Provider** 均繼承自 `MarketDataProvider` 基底類別（`src/data/providers/base.py`），實作統一的 `fetch_current_prices()`, `fetch_historical()`, `fetch_info()` 等介面。
...
- **代碼用途**: 即時監控盤中異動。已實作 `PolygonStreamClient` 支援秒級 WebSocket 串流。
...
- **代碼用途**: 新聞噪音過濾與 Sentiment 引擎燃料。
- **優化實作**: v5.0 已實作 `TiingoProvider`。
