...
17. 開發起飛準則 (Agent Preflight Standards): **嚴禁盲目開發**。Agent 在接收開發要求時，必須先閱讀 `wiki/` 內的架構觀點文件，了解全局架構；同時必須具備「UI/UX 共感」，主動實作前端設定介面，且沿用系統既有模式 (如 `SettingsService` 或 Base Class)。詳情參見 `.agent/rules/agent-preflight-standards.md` 與 `/feature-implementation-preflight` workflow。
